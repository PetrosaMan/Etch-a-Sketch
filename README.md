...
Note: The mouseover event listener is inside the for loop to create an event handler for each individual square as they’re created. Each grid square needs its own listener so it can respond when the mouse hovers over it.

If  the listener was put outside the loop, it wouldn't apply to the individual square as they have not been created yet. Think of the loop as setting up each square with its own behavior as it’s being added to the grid.
